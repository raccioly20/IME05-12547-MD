...
9)  INICIO
10) TERMINO
11) PROFUNDIDADE_MEDIDA_M

Para selecionar colunas usamos a função `select`

```{r selecionando}
## Vamos selecionar as colunas listadas acima
pocos_01 <- pocos %>% select(POCO,OPERADOR,ESTADO,BACIA, CAMPO,TERRA_MAR,CATEGORIA,SITUACAO, INICIO, TERMINO, PROFUNDIDADE_MEDIDA_M)
head(pocos_01)
summary(pocos_01)
```
...
ggplot(pocos_04, aes(x=PROFUNDIDADE_MEDIDA_M, y=TPERF)) + 
  geom_point()
cor(pocos_04$PROFUNDIDADE_MEDIDA_M, pocos_04$TPERF)
```
