...
library(e1071)
set.seed(1234)
y_rotulos <- conj_treino$inadimplente
knn_outra <- tune.knn(conj_treino[,-1], y_rotulos, k = 1:50)
knn_outra
summary(knn_outra)
```
...
```{r}
set.seed(1234)
knn1_t <- knn3(inadimplente ~ balanco + receita + estudante, data = conj_treino, k = 35)
knn1_t
y_chapeu_knn1_t <- predict(knn1_t, conj_teste, type = "class")
# Matriz de confusão para valiar os resultados
confusionMatrix(y_chapeu_knn1_t, conj_teste$inadimplente, positive="Sim") 
## ROC
library(pROC)
prev_knn1_t <- predict(knn1_t, conj_teste, type = "prob")
roc_knn1_t <- roc(conj_teste$inadimplente ~ prev_knn1_t[,2], plot = TRUE,
                  print.auc=FALSE, col="black", legacy.axes=TRUE)
as.numeric(roc_knn1_t$auc)
```
